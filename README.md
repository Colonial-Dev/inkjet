...
The value of these variables doesn't matter - they just have to be set. I also recommend running `cargo build -vv` when redownloading languages, so the script's progress is visible.

(Also note that `INKJET_REDOWNLOAD_LANGS` will probably only work on Linux (and possibly other *nix systems) - some of the languages have miniature "setup scripts" defined that assume the existence of programs like `sed`.)
